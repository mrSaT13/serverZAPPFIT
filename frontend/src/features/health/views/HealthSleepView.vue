...
import { Moon, Plus, Target } from '@lucide/vue'

import type { HealthInterval, SleepEntry } from '@/features/health/types'

import HealthSleepChart from '@/features/health/components/HealthSleepChart.vue'
import HealthSleepFormDialog from '@/features/health/components/HealthSleepFormDialog.vue'
import HealthSleepListItem from '@/features/health/components/HealthSleepListItem.vue'
import HealthSleepTargetDialog from '@/features/health/components/HealthSleepTargetDialog.vue'
...
/** Selectable time windows for the sleep history, in display order. */
...
const targetSeconds = computed(() => targets.value?.sleepSeconds ?? null)

const listQuery = useSleepEntriesQuery(page, recordsPerPage, interval)

/** Sleep entries on the current page (newest first). */
...
const showChart = computed(
  () =>
    ...
    !isPending.value &&
    !isError.value &&
    entries.value.some((entry) => entry.totalSleepSeconds !== null),
)
...
function openEdit(entry: SleepEntry): void {
...
const entryToDelete = ref<SleepEntry | null>(null)
const deleteMutation = useDeleteSleepEntryMutation()

function openDelete(entry: SleepEntry): void {
...
function confirmDelete(): void {
  ...
  deleteMutation.mutate(entry.id, {
    onSuccess: () => {
      ...
      toasts.success(t('health.sleep.delete.success'))
    },
    onError: () => {
      toasts.error(t('health.sleep.delete.error'))
    },
  })
}
</script>
...
<template>
  <section class="flex flex-col gap-3">
    <header class="flex flex-col gap-1">
      <h1 class="text-page-title">{{ t('health.sleep.title') }}</h1>
      <p class="text-body">{{ t('health.sleep.subtitle') }}</p>
    </header>
    ...
    <div class="flex flex-wrap items-center gap-2">
      <Button @click="openAdd">
        ...
        {{ t('health.sleep.logButton') }}
      </Button>
      <Button variant="outline" :disabled="!targets" @click="openTarget">
        <Target class="size-4" aria-hidden="true" />
        {{ t('health.sleep.targetButton') }}
      </Button>
      <div class="ms-auto w-full sm:w-36">
        <Select v-model="interval" :aria-label="t('health.sleep.interval.label')">
          <option v-for="option in INTERVAL_OPTIONS" :key="option" :value="option">
            {{ t(`health.sleep.interval.${option}`) }}
          </option>
        </Select>
      </div>
    </div>

    <HealthSleepChart v-if="showChart" :entries="entries" :target-seconds="targetSeconds" />
    ...
    <ListPanel
      ...
      :is-empty="isEmpty"
      :show-header="false"
      :error-title="t('health.sleep.error.title')"
      :error-description="t('health.sleep.error.description')"
      :retry-label="t('health.sleep.error.retry')"
      paginated
    ...
    >
      ...
      <template #empty>
        <EmptyState
          :title="t('health.sleep.empty.title')"
          :description="t('health.sleep.empty.description')"
          variant="first-time"
        >
          <template #icon>
            <Moon class="size-8" aria-hidden="true" />
          </template>
          <template #action>
            <Button @click="openAdd">
              <Plus class="size-4" aria-hidden="true" />
      ...
      <ul class="divide-y divide-border">
        <li v-for="entry in entries" :key="entry.id">
          <HealthSleepListItem :entry="entry" @edit="openEdit" @delete="openDelete" />
        </li>
      </ul>
    </ListPanel>

    <HealthSleepFormDialog
      v-model:open="isFormOpen"
      :entry="editingEntry"
      @success="onDialogSuccess"
      @error="onDialogError"
    />
    <HealthSleepTargetDialog
      v-if="targets"
    ...
    <ConfirmDialog
      ...
      :title="t('health.sleep.delete.title')"
      :description="t('health.sleep.delete.body')"
      :confirm-label="t('health.sleep.delete.confirm')"
      :cancel-label="t('health.sleep.delete.cancel')"
      :close-label="t('health.sleep.delete.close')"
