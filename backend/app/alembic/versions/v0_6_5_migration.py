...
down_revision: str | None = "65a0f1d72997"
...
def upgrade() -> None:
    ...
    if dialect_name == "postgresql":
        ...
        index_garminconnect_activity_id_exists = conn.execute(
            text("""
            SELECT EXISTS (
                SELECT 1
                FROM pg_class c
        ...
        index_created_at_exists = conn.execute(
            text("""
            SELECT EXISTS (
                SELECT 1
                FROM pg_class c
    ...
    op.add_column(
        "activities",
        sa.Column("timezone", sa.String(length=250), nullable=True, comment="Activity timezone (May include spaces)"),
    )
    op.alter_column(
        "activities",
        "start_time",
    ...
    op.add_column(
        "gear",
        sa.Column(
            "initial_kms", sa.DECIMAL(precision=11, scale=3), nullable=False, comment="Initial kilometers of the gear"
        ),
    )
    op.alter_column(
        "gear",
    ...
    op.add_column(
        "health_data", sa.Column("date", sa.Date(), nullable=True, comment="Health data creation date (date)")
    )
    ...
    op.alter_column(
        "health_data", "date", nullable=False, comment="Health data creation date (date)", existing_type=sa.Date()
    )
    op.add_column(
        "health_data",
        sa.Column("bmi", sa.DECIMAL(precision=10, scale=2), nullable=True, comment="Body mass index (BMI)"),
    )
    op.add_column(
        "health_data",
        sa.Column(
...
def downgrade() -> None:
    # ### commands auto generated by Alembic - please adjust! ###
    op.add_column(
        "health_data", sa.Column("created_at", sa.DATE(), nullable=True, comment="Health data creation date (datetime)")
    )
