...
class TestGetSetupOptions:
    """Test suite for get_setup_options function."""

    def test_get_setup_options_returns_theme_and_language_choices(self):
        """Test that setup options ship the theme/language pickers and brand."""
        # Act
        result = server_settings_utils.get_setup_options()

        # Assert
        assert isinstance(result, server_settings_schema.SetupOptions)
        assert result.brand_name == "ZAPFIT"
        assert [option.value for option in result.themes] == ["light", "dark", "system"]
        assert len(result.languages) > 0
        assert "en" in [option.code for option in result.languages]
        assert "ru" in [option.code for option in result.languages]

    def test_get_setup_options_theme_labels_are_human_readable(self):
        """Test that every theme option carries a non-empty label."""
        # Act
        result = server_settings_utils.get_setup_options()

        # Assert
        assert all(option.label.strip() for option in result.themes)
        assert all(option.label.strip() for option in result.languages)
