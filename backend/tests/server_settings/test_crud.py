...
def _make_setup_complete_payload() -> server_settings_schema.SetupCompletePayload:
    """Builds a full SetupCompletePayload with sane defaults."""
    return server_settings_schema.SetupCompletePayload(
        id=1,
        units=server_settings_schema.Units.METRIC,
        public_shareable_links=False,
        public_shareable_links_user_info=False,
...
class TestApplySetupComplete:
    """Test suite for apply_setup_complete function."""

    @patch("server_settings.crud._transform_server_settings")
    @patch("server_settings.crud._get_server_settings_model_or_404")
    def test_apply_setup_complete_success(self, mock_get_model, mock_transform, mock_db):
        """Test successful setup completion applies the payload."""
        # Arrange
        mock_existing_settings = MagicMock(spec=server_settings_models.ServerSettings)
        mock_existing_settings.id = 1
        mock_existing_settings.units = 1
        mock_existing_settings.default_theme = "light"
        mock_existing_settings.default_language = "en"
        mock_existing_settings.brand_name = "ZAPFIT"

        mock_get_model.return_value = mock_existing_settings
        mock_transform.return_value = mock_existing_settings

        # Act
        result = server_settings_crud.apply_setup_complete(_make_setup_complete_payload(), mock_db)

        # Assert
        assert result == mock_existing_settings
        assert mock_existing_settings.default_theme == "dark"
        assert mock_existing_settings.default_language == "ru"
        assert mock_existing_settings.brand_name == "ZAPFIT"
        mock_db.commit.assert_called_once()
        mock_db.refresh.assert_called_once_with(mock_existing_settings)

    @patch("server_settings.crud._transform_server_settings")
    @patch("server_settings.crud._get_server_settings_model_or_404")
    def test_apply_setup_complete_always_marks_completed(self, mock_get_model, mock_transform, mock_db):
        """Test that setup_completed is forced to True on success."""
        # Arrange
        mock_existing_settings = MagicMock(spec=server_settings_models.ServerSettings)
        mock_existing_settings.setup_completed = False
        mock_existing_settings.units = 1
        ...
        # Act
        server_settings_crud.apply_setup_complete(_make_setup_complete_payload(), mock_db)

        # Assert
        assert mock_existing_settings.setup_completed is True

    @patch("server_settings.crud._get_server_settings_model_or_404")
    def test_apply_setup_complete_not_found(self, mock_get_model, mock_db):
        """Test setup completion when settings don't exist."""
        # Arrange
        mock_get_model.side_effect = HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Server settings not found.",
