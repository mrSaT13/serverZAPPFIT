# ruff: noqa: E402  (shim import after sys.path setup, below)
import contextlib
import os
import sys
...
# Install the Windows `resource` shim (sitecustomize.py in the backend root)
# before any app module import, so safeuploads can load on win32.
BACKEND_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
if BACKEND_DIR not in sys.path:
    sys.path.insert(0, BACKEND_DIR)
import sitecustomize  # noqa: F401  (imported for its install_resource_shim side effect)
